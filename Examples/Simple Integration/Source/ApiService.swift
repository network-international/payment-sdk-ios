...
/// Handles the two server-side API calls needed for payment integration:
///   1. **Auth token** — exchanges API key for a Bearer token via the identity endpoint
///   2. **Create order** — sends order details to the gateway and returns an OrderResponse
///
/// In production, both calls should happen on your backend to keep the API key secret.
/// This demo calls them from the client for simplicity.
class ApiService {
    func getAuthToken(environment: Environment, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: authRequest) { (data, response, error) in
            guard let data = data else {
                print("Auth error: no data returned")
            ...
            getAuthToken(environment: environment) { result in
                ...
                case .success(let accessToken):
                    ...
                    URLSession.shared.dataTask(with: orderRequest) { (data, response, error) in
                        guard let data = data else {
                            print("Order error: no data returned")
                ...
                case .failure(let error):
                    print("Order error: auth failed - \(error.localizedDescription)")
                    completion(.failure(error))
                }
            }
        } else {
