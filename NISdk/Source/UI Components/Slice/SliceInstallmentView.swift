...
final class SliceBannerUIView: UIView {
    ...
    @objc private func handleLearnMoreTap() {
        onLearnMoreTapped?()
    }
...
// MARK: - SliceLearnMoreViewController
//
// Informational modal presented when the user taps "Learn more" on the Slice banner.

final class SliceLearnMoreViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        scrim.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(dismissSelf)))
        ...
        card.layer.shadowOffset = CGSize(width: 0, height: 32)
        card.layer.shadowRadius = 64
        ...
        let titleColor = UIColor(red: 0x07/255, green: 0x07/255, blue: 0x07/255, alpha: 1)

        let titleLabel = UILabel()
        titleLabel.font = .systemFont(ofSize: 20, weight: .medium)
        titleLabel.textColor = titleColor
        titleLabel.numberOfLines = 0
        // Line-height per Figma: 32pt against 20pt size.
        let titleParagraph = NSMutableParagraphStyle()
        titleParagraph.minimumLineHeight = 32
        titleParagraph.maximumLineHeight = 32
        titleLabel.attributedText = NSAttributedString(
            string: "Split your payment in seconds with Slice",
            attributes: [
        ...
        let bodyColor = UIColor(red: 0x4A/255, green: 0x4A/255, blue: 0x4A/255, alpha: 1)
        let bodyLabel = UILabel()
        bodyLabel.font = .systemFont(ofSize: 14, weight: .regular)
        bodyLabel.textColor = bodyColor
        bodyLabel.numberOfLines = 0
        let bodyParagraph = NSMutableParagraphStyle()
        bodyParagraph.minimumLineHeight = 22.4
        bodyParagraph.maximumLineHeight = 22.4
        bodyLabel.attributedText = NSAttributedString(
            string: "Enter your credit card number above to check if Slice is available on your card.",
            attributes: [
                .font: bodyLabel.font!,
        ...
        banksLabel.text = "Participating banks"
        banksLabel.font = PgType.bodyRowSubtitle
        banksLabel.textColor = PgColors.textMuted
        banksLabel.translatesAutoresizingMaskIntoConstraints = false

        let sdkBundle = NISdk.sharedInstance.getBundle()
        let fabLogo = SliceBannerUIView.makeBankLogo(named: "bankLogoFab", in: sdkBundle, width: 40, height: 16)
        let adcbLogo = SliceBannerUIView.makeBankLogo(named: "bankLogoAdcb", in: sdkBundle, width: 54, height: 16)
        let enbdLogo = SliceBannerUIView.makeBankLogo(named: "bankLogoEnbd", in: sdkBundle, width: 60, height: 16)
        // EI's source PNG is square; give it equal width/height so the wordmark renders at a
        // comparable visual size to the other (landscape) logos instead of shrinking to fit
        // a 16pt-tall row.
        let eiLogo = SliceBannerUIView.makeBankLogo(named: "bankLogoEi", in: sdkBundle, width: 84, height: 84)

        let bankRow = UIStackView(arrangedSubviews: [fabLogo, adcbLogo, enbdLogo, eiLogo, UIView()])
        bankRow.axis = .horizontal
        bankRow.alignment = .center
        bankRow.spacing = 14
        bankRow.translatesAutoresizingMaskIntoConstraints = false
        ...
        footerParagraph.minimumLineHeight = 16
        footerParagraph.maximumLineHeight = 16
        footerLabel.attributedText = NSAttributedString(
            string: "Supported by leading UAE banks & cards",
            attributes: [
        ...
        card.addSubview(banksLabel)
        card.addSubview(bankRow)
        ...
        NSLayoutConstraint.activate([
            ...
            closeButton.topAnchor.constraint(equalTo: card.topAnchor, constant: 20),
            ...
            banksLabel.topAnchor.constraint(equalTo: bodyLabel.bottomAnchor, constant: 16),
            banksLabel.leadingAnchor.constraint(equalTo: card.leadingAnchor, constant: 20),
            banksLabel.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -20),

            bankRow.topAnchor.constraint(equalTo: banksLabel.bottomAnchor, constant: 12),
            bankRow.leadingAnchor.constraint(equalTo: card.leadingAnchor, constant: 20),
            bankRow.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -20),

            footerLabel.topAnchor.constraint(equalTo: bankRow.bottomAnchor, constant: 16),
            footerLabel.leadingAnchor.constraint(equalTo: card.leadingAnchor, constant: 20),
            footerLabel.trailingAnchor.constraint(equalTo: card.trailingAnchor, constant: -20),
            footerLabel.bottomAnchor.constraint(equalTo: card.bottomAnchor, constant: -20),
        ])
    }
